...
BoxContainer<T>::BoxContainer( BoxContainer<T>&& source)
{
	...
	std::cout << "BoxContainer Move constructor.....";
	std::cout << std::endl;
...
{
	std::cout << "BoxContainer object with size : " << m_size  <<" destroyed";
	std::cout << std::endl;
	delete[] m_items;
}
...
void BoxContainer<T>::add(const T& item){
	std::cout << "Add copy version called...";
	std::cout << std::endl;
...
void BoxContainer<T>::add( T&& item){
	std::cout << "Add move version called...";
	std::cout << std::endl;
	if (m_size == m_capacity)
		//expand(m_size+5); // Let's expand in increments of 5 to optimize on the calls to expand
		expand(m_size + EXPAND_STEPS);
	m_items[m_size] = std::move(item);//Move
	++m_size;
}
